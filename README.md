...
- **`use_absolute_urls`** (default: `False`): Will use absolute URLs when creating the links for pages.
...
- **`use_absolute_urls`** (default: `None`): Will use absolute URLs when creating the links for pages. When `None` is specified we check the template context for `use_absolute_urls`, this allows the sub menu to honor the value specified for any parent menus. If `use_absolute_urls` is not in the context and not passed to the tag, it defaults to `False`.
...
- **`use_absolute_urls`**: A boolean indicating whether menu items should be linked using absolute URLs.
