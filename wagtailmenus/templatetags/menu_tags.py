...
def get_attrs_from_context(context, guess_tree_position=True):
    ...
    # Look for values added by `wagtailmenus_params_helper`.
    section_root = request.META.get('CURRENT_SECTION_ROOT')
    ancestor_ids = request.META.get('CURRENT_PAGE_ANCESTOR_IDS', None)

    # If there are still values missing, try for values previously added to
    # the context by this method
    if section_root is None:
        section_root = context.get('wagtailmenus_section_root', None)
    if ancestor_ids is None:
        ancestor_ids = context.get('wagtailmenus_current_ancestor_ids', [])
    if current_page is None:
        current_page = context.get('wagtailmenus_current_page', None)
        identified_page = context.get('wagtailmenuse_identified_page', None)

    best_match = current_page or identified_page
    if guess_tree_position and not best_match:
        ...
        while path_components and not identified_page:
            try:
                ...
                # Set context values so they can be picked up by other tags
                context['wagtailmenuse_identified_page'] = identified_page
                context['wagtailmenus_current_ancestor_ids'] = ancestor_ids
                if first_run:
                    # A page was found matching the exact path, so it's
                    # safe to assume it's the 'current page'
                    current_page = identified_page
                    context['wagtailmenus_current_page'] = current_page
            except Http404:
                # No match found, so remove a path component and try again
                path_components.pop()
            first_run = False  # Don't use non-exact matches as 'current_page'

    if guess_tree_position and not section_root and best_match:
        ...
        if best_match.depth > SECTION_ROOT_DEPTH:
            ...
            section_root = site.root_page.get_descendants().ancestor_of(
                best_match, inclusive=True
            ).filter(depth__exact=SECTION_ROOT_DEPTH).first()
            context['wagtailmenus_section_root'] = section_root
        ...
        if page:
            ...
            if (
                ...
                if (
                    ...
                    if type(page) is Page:
                        page = page.specific
                    has_submenu_items = page.has_submenu_items(
                        current_page=current_page,
                        allow_repeating_parents=allow_repeating_parents,
                        original_menu_tag=original_menu_tag,
                        menu_instance=menu_instance)
                    ...
                    has_submenu_items = menu_instance.page_has_children(page)
                else:
                    has_submenu_items = page.get_children().filter(
                        live=True, expired=False, show_in_menus=True,
                    ).exists()

            setattr(item, 'has_children_in_menu', has_submenu_items)
